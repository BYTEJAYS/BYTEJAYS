...
RED = "#f85149"
PURPLE = "#b083f0"
LANG_COLORS = {           # GitHub's official language dot colors
...
def wrap_words(text: str, limit: int) -> list[str]:
    """Wrap plain text into lines of <= limit chars, breaking on spaces."""
...
def window_chrome(h: int, title: str) -> list[str]:
    """Terminal window frame: rounded card, title bar, traffic lights."""
    return [
...
def anim_css(n_cmd: int, out_base: float, blink_start: float) -> str:
    """Shared animation rules; per-element delays come from inline styles."""
    return (
        "<style>"
        ".o{opacity:0;animation:o .01s steps(1) both}"
        "@keyframes o{to{opacity:1}}"
        f"#cur{{animation:"
        f"r {n_cmd * TYPE_STEP:.2f}s steps({n_cmd}) {TYPE_DELAY:.2f}s both,"
        f"h .01s steps(1) {out_base:.2f}s forwards}}"
        f"@keyframes r{{to{{transform:translateX({n_cmd * CW:.1f}px)}}}}"
...
def typed_prompt(body: list, cfg: dict, cmd: str, top: float) -> tuple:
    """Emit prompt + char-by-char typed command + riding cursor.

    Returns (cmd_x, prefix_runs, out_base) — where out_base is when the
    command "runs" and output may start printing.
    """
    ...
    return cmd_x, prefix, TYPE_DELAY + len(cmd) * TYPE_STEP + PRINT_PAUSE


def svg_doc(h: int, label: str, css: str, chrome: list, body: list) -> str:
...
# ------------------------------------------------------------------ build --
def build(cfg: dict, stats: dict | None) -> str:
    ...
    h = int(end_y + PAD - 4)
    return svg_doc(
        h, f'{cfg["brand_name"]} — terminal-style developer profile card',
        anim_css(len(cmd), out_base, blink_start),
        window_chrome(h, cfg["terminal_title"]), body)


# --------------------------------------------------------- projects panel --
def build_projects(cfg: dict, stats: dict | None) -> str | None:
    """Second terminal panel: `ls ~/projects`, one project written at a time."""
    projects = (stats or {}).get("projects") or {}
    featured = [n for n in cfg.get("featured_projects", []) if n in projects]
    if not featured:
        return None
    ...
    y = top + LH + 12
    ry = y
    desc_chars = int((W - 2 * PAD) / CW) - 2
    for name in featured:
        p = projects[name]
        # title row: name left, stars + language dot right-aligned
        right = []
        if p["stars"]:
            right += [(f"★ {p['stars']}", ORANGE), ("   ", MUTED)]
        if p["language"]:
            right += [("● ", LANG_COLORS.get(p["language"], FAINT)),
                      (p["language"], MUTED)]
        body.append(text_el(PAD, ry, [("▸ ", GREEN), (name.lower(), CYAN)],
                            weight="bold"))
        if right:
            rlen = sum(len(t) for t, _ in right)
            body.append(text_el(W - PAD - rlen * CW, ry, right))
        wipe(ry - 13, 17)
        ry += LH
        # description, wrapped to at most two muted lines
        for ln in wrap_words(p["description"], desc_chars)[:2]:
            body.append(text_el(PAD + 2 * CW, ry, [(ln, MUTED)]))
            wipe(ry - 13, 17)
            ry += LH
        ry += LH * 0.5
    ry -= LH * 0.5
    ...
    h = int(end_y + PAD - 4)
    return svg_doc(
        h, "Featured projects — animated terminal listing",
        anim_css(len(cmd), out_base, write_end),
...
def build_button(label: str) -> str:
    ...
    runs = [("❯ ", GREEN), (label, TEXT)]
    n = sum(len(t) for t, _ in runs)
    bw, bh = int(n * CW + 30), 34
    return (
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{bw}" height="{bh}" '
        f'viewBox="0 0 {bw} {bh}" role="img" aria-label="{escape(label)}">\n'
        f'<rect x="0.5" y="0.5" width="{bw - 1}" height="{bh - 1}" rx="8" '
        f'fill="{BG_BAR}" stroke="{BORDER}"/>\n'
        + text_el(15, 21.5, runs) + "\n</svg>\n"
    )


BUTTONS = ["email", "linkedin", "portfolio"]


def write_out(path: Path, content: str) -> None:
    ...
    for path, make in targets:
        try:
            ...
            if path.exists():
                ...
                print(f"error: {path.name} failed, keeping existing ({e})",
                      file=sys.stderr)
                continue
            raise
        if svg is None:
            print(f"info: skipped {path.name} (no data)", file=sys.stderr)
            continue
        write_out(path, svg)
